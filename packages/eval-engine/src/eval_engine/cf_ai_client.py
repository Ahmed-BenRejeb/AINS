...
# the 30s backoff budget (90s total) and still fails, so we fail fast on it instead
# of treating it as transient — otherwise `make eval` (k×runs judge calls) would
# hang for many minutes per call before giving up.
