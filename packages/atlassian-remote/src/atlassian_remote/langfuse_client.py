"""Shared Langfuse observability client (UC3 remote backend).

A single, lazily-initialised :class:`langfuse.Langfuse` client is shared across the
service so every LLM call and vector search lands in one Langfuse project.
Initialisation is env-driven (``LANGFUSE_PUBLIC_KEY`` / ``LANGFUSE_SECRET_KEY`` /
``LANGFUSE_HOST``) and **best-effort**: when those vars are unset the helpers
degrade to no-ops, so tracing never breaks an incident analysis (the same
best-effort posture as recording + eval in the Phase 4 loop).
...
logger = logging.getLogger("atlassian_remote.langfuse")
...
def start_generation(name: str, model: str, input: Any) -> LangfuseGeneration | None:
    ...
    Args:
        name: Human-readable observation name (e.g. ``"rca-generation"``).
