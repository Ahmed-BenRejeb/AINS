...
> All three use cases (UC1 eval engine · UC2 flight recorder · UC3 Atlassian Rovo agent) are
> **built, live-validated, and deployed**. Every spec Must/Should is met. The end-to-end loop runs
> on the Azure VM; the Forge app is installed on Jira + Confluence; the dashboard is live with all
> screens. Phase 5 differentiators (drift, evaluator-quality, K8s/KEDA/Helm, observability, chaos,
> interpretability) are done. **167 Python + 25 TS tests green; `mypy --strict` + `ruff` clean.**
...
| Workers AI (overflow) | Account `aff2c0701d26924782e108174fa9d317` / token `cfut_DZ…` — set via `CF_AI_ACCOUNT_ID`/`CF_AI_API_TOKEN` for a fresh 10k-neuron/day budget when the primary account is exhausted |
...
| Phase 4 — Integration | ✅ Done (live-validated) | **End-to-end loop wired:** `POST /analyze` generates a `run_id`, records every RCA-gen CF Workers AI call into a MinIO cassette via `AsyncRecordingTransport` (hash-chain audit → D1), writes a `run_manifests` row, then `POST`s the `run_id` to eval-engine `:8000/evaluate` (which loads the cassette, judges, and files the Jira Incident on fail/flag). Response envelope gains `run_id` + `eval_verdict` + `replay_link`. Internal calls use localhost (eval `:8000`, MinIO `:9090`), never tunnel URLs. `tests/integration/test_full_loop.py` proves it end-to-end with all boundaries mocked. **Live-validated on the VM** (2026-06-20): `POST /analyze AO-1` → 200 with full `eval_verdict`, cassette in MinIO, `run_manifests` row, eval-engine filed an AO Incident; xqdrant seeded (101 incidents + 11 runbooks) so retrieval returns evidence. **Forge cloud deploy DONE** (see Phase 3). |
| Phase 5 — Differentiators | ✅ Done | **Dashboard** (`packages/dashboard`): all 7 screens live — `/` overview · `/runs` · `/runs/[run_id]` · `/verdicts/[run_id]` · `/replay/[run_id]` (with mid-replay injection + before/after diff) · `/drift` · `/reliability` (drift + evaluator-quality side by side). `?mock=true` on every page with automatic live→mock fallback. **Drift detection** (`POST /drift`, `drift/detector.py`) + **evaluator quality** (`GET /evaluator-quality/demo`, Cohen's κ) both built and live-verified. **Langfuse tracing** live (all 3 Python services). **K8s/Helm/KEDA** (`deploy/`): Kubernetes manifests, Helm chart, KEDA autoscaler, Prometheus/Grafana observability stack, ChaosMesh chaos experiments. **Embedding interpretability** (`interpretability/`): 5-step pipeline (vocabulary → segmentation → geometry → perturbation → attribution). |

**✅ All phases complete.** The end-to-end loop is live-validated. All spec Musts/Shoulds are met (see `docs/spec_response.md`). The demo-day path is documented in `docs/validation_guide.md` — it works without the Rovo agent being visually enabled in Atlassian (backend + dashboard + real Jira artifacts path). Reuse `normalize_request` / `hash_step_key` from `trace_core` for cassette keys — do not redefine them.
