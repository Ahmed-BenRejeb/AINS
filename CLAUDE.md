...
| Phase 3 — UC3 Atlassian Agent | 🟦 In progress | Cores built: `atlassian-remote` (FastAPI :8080 — `/analyze` `/search` `/embed` `/health`; embeddings, xqdrant search, RCA drafting) and `atlassian-agent` (Forge Rovo agent + 6 actions). `make test-uc3` green (30 Python + 19 TS tests); ruff/black/isort/mypy --strict + tsc/eslint/prettier clean. **Not yet deployed to Forge** (needs `forge register`/`deploy` + Atlassian login) |
