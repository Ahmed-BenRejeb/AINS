...
| Phase 3 — UC3 Atlassian Agent | 🟦 In progress | Cores built: `atlassian-remote` (FastAPI :8080 — `/analyze` `/search` `/embed` `/health`; embeddings, xqdrant search, RCA drafting; `/analyze` now records + evals — see Phase 4) and `atlassian-agent` (Forge Rovo agent + 6 actions). `make test-uc3` Python green (32 tests); ruff/black/isort/mypy --strict + check-docs clean. TS jest unchanged (19 tests; not re-run here — pnpm install blocked by a `minimumReleaseAge` lockfile policy). **`forge register` done** (`manifest.yml` has a real app ARI); remaining = `forge deploy` + `forge install` |
