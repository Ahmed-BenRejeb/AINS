...
| Phase 1 — UC2 Flight Recorder | 🟦 In progress | Core built: cassette (now carries full `records` for non-lossy traces), RecordingTransport **+ AsyncRecordingTransport**, @record_tool, hash-chain audit, D1/MinIO clients, replay + bisect, `manifest.write_run_manifest`, FastAPI on 8001. `make test-uc2` green (30 tests); ruff/black/isort/mypy --strict clean on the package |
| Phase 2 — UC1 Eval Engine | 🟦 In progress | Core built: cf_ai_client, safety pre-filter (Llama Guard), code grader, calibrated LLM judge (mandatory position-bias calibration), DAG failure attribution, pass^k (all() not any()), verdict reporter (files AO Jira issue, type id 10013, no priority/labels), FastAPI on 8000. `trace_loader` now loads the **full MinIO cassette** (D1 previews are fallback). `make test-uc1` green (22 tests); ruff/black/isort/mypy --strict clean |
| Phase 3 — UC3 Atlassian Agent | 🟦 In progress | Cores built: `atlassian-remote` (FastAPI :8080 — `/analyze` `/search` `/embed` `/health`; embeddings, xqdrant search, RCA drafting; `/analyze` now records + evals — see Phase 4) and `atlassian-agent` (Forge Rovo agent + 6 actions). `make test-uc3` Python green (31 tests); ruff/black/isort/mypy --strict + check-docs clean. TS jest unchanged (19 tests; not re-run here — pnpm install blocked by a `minimumReleaseAge` lockfile policy). **Not yet deployed to Forge** (needs `forge register`/`deploy` + Atlassian login) |
| Phase 4 — Integration | 🟦 In progress | **End-to-end loop wired:** `POST /analyze` generates a `run_id`, records every RCA-gen CF Workers AI call into a MinIO cassette via `AsyncRecordingTransport` (hash-chain audit → D1), writes a `run_manifests` row, then `POST`s the `run_id` to eval-engine `:8000/evaluate` (which loads the cassette, judges, and files the Jira Incident on fail/flag). Response envelope gains `run_id` + `eval_verdict` + `replay_link`. Internal calls use localhost (eval `:8000`, MinIO `:9090`), never tunnel URLs. `tests/integration/test_full_loop.py` proves it end-to-end with all boundaries mocked. Remaining: live VM smoke test + Forge deploy |
| Phase 5 — Differentiators | ⬜ Not started | |

**⚡ Next task: live smoke test of the Phase 4 loop + deploy UC3 to Forge.** The cross-cutting integration glue is now wired and unit/integration-tested locally (Phase 4 row above). Remaining work is to (1) run the loop against the **live** VM services — start `eval-engine :8000`, `flight-recorder :8001`, MinIO `:9090`, xqdrant `:6333`, then `POST /analyze` a real AO incident and confirm a cassette lands in `sentinel-cassettes`, a `run_manifests` row appears, and an `EvalVerdict` (plus a Jira Incident on fail/flag) comes back; and (2) `forge register` + `forge deploy --environment development` + `forge install`, setting `FORGE_REMOTE_URL`/`FORGE_REMOTE_SECRET` Forge variables. Reuse `normalize_request` / `hash_step_key` from `trace_core` for cassette keys — do not redefine them.
