...
| CF daily neuron allocation 429 is **non-transient** | When the 10k/day free budget is spent, CF returns `429` with error **code 4006** ("daily free allocation of 10,000 neurons"). The plain 30s×3 retry just burned 90s and still failed — hanging `/search`/`/analyze` past the Forge 25s timeout, then surfacing as a bare **500**. The free budget is a **rolling window**, not a clean midnight reset (still spent at 00:29 UTC). | `cf_ai_client._post` now **fails fast** on a 4006/"neurons" 429 (no backoff) in **both** packages; `api.py` (eval-engine + atlassian-remote) maps any upstream `httpx.HTTPStatusError` to a clean **503** ("retry later"). Verified live: `/search`→503 in 0.11s, `/analyze`→503 in 5s. Demo from cassettes via deterministic `/replay` (0 live calls) when budget is short |
...
| Foundation — `trace-core` | ✅ Done | Shared contract complete: constants, Pydantic v2 schemas, hash_utils, OTel GenAI span helpers, schema.ts mirror. `make test-core` green (20 tests); ruff/black/isort/mypy --strict clean on the package |
