...
> - **Status:** `pnpm --filter dashboard build` + `typecheck` + `lint` all green; all 6 routes smoke-tested → 200 in mock mode (and real-mode degrades to mock-fallback). Added `packages/dashboard` to `pnpm-workspace.yaml`. Not done: no unit-test framework wired (the `test` script runs `tsc --noEmit`), and the live URLs return 403 from this sandbox's egress (so only mock-fallback is exercisable here — live data will show on the VM/demo box).
