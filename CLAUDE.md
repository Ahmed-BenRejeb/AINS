...
| UC3 | Semantic duplicate auto-link gated by `DUPLICATE_CONFIDENCE_THRESHOLD` (0.85), stricter than the 0.70 RCA gate | Auto-mutating Jira (issue link + comment) is higher-stakes than drafting a comment; below the gate the resolver surfaces candidates for a human |
...
| Phase 3 — UC3 Atlassian Agent | 🟦 In progress | Cores built: `atlassian-remote` (FastAPI :8080 — `/analyze` `/duplicates` `/search` `/embed` `/health`; embeddings, xqdrant search, RCA drafting, semantic duplicate resolver) and `atlassian-agent` (Forge Rovo agent + 7 actions, incl. `resolve-duplicate` → auto-links Jira issues). `make test-uc3` green (38 Python + 25 TS tests); ruff/black/isort/mypy --strict + tsc/eslint/prettier clean. **Not yet deployed to Forge** (needs `forge register`/`deploy` + Atlassian login) |
| Phase 4 — Integration | ⬜ Not started | |
| Phase 5 — Differentiators | ⬜ Not started | |

**⚡ Next task: deploy UC3 and wire the cross-cutting integration glue.** The UC3 cores (atlassian-remote + atlassian-agent) are built and tested locally; remaining UC3 work is `forge register` + `forge deploy --environment development` + `forge install`, setting `FORGE_REMOTE_URL`/`FORGE_REMOTE_SECRET` Forge variables, and a live `/analyze` smoke test against the seeded xqdrant collections. Still open from UC1/UC2: record a real agent run end-to-end against live MinIO/D1, write `run_manifests` rows, and wire the eval-engine `trace_loader` to the live flight recorder so `POST /evaluate {run_id}` reconstructs full traces. Reuse `normalize_request` / `hash_step_key` from `trace_core` for cassette keys — do not redefine them.
...
> - **`make check` is not yet green repo-wide** — blocked only by items outside trace-core: pre-existing lint errors in `scripts/seed_atlassian.py` + `scripts/run_synthetic_eval.py`, and the `dashboard` TS package, which still has no `package.json` (Phase 5). trace-core itself passes ruff/black/isort/mypy/pytest.
...
> - **Shape:** Forge app — `forge.yml` declares one `rovo:agent` plus 6 `action` + 6 `function` modules; handlers are re-exported from `src/index.ts` (`handler: index.<fn>`). `app.id` is a placeholder — run `forge register` before the first deploy.
> - **6 actions:** `fetch-incident` (native Jira read) · `search-similar-incidents` / `search-runbooks` (→ remote `/search`) · `post-rca-comment` (→ remote `/analyze` → ADF Jira comment) · `draft-pir-page` (→ `/analyze` → Confluence page via ADF/`atlas_doc_format`) · `flag-knowledge-gap` (native AO issue, id 10013). Not every action hits the remote — `fetch` and `flag` are Atlassian-native by design.
...
> - **Tooling:** added a root `pnpm-workspace.yaml` so `pnpm --filter atlassian-agent <script>` resolves. `pnpm --filter atlassian-agent test` green (19 jest tests; `@forge/api` is a manual mock under `tests/__mocks__`); `tsc --noEmit` (strict) + eslint + prettier all clean.
> - **Not verifiable in this env:** `forge lint`/`forge deploy`/`forge install` (Forge CLI not installed, no Atlassian login) and the live "agent appears in ahmedains.atlassian.net" check. `make check`/`make test` still fail on the `dashboard` package (no `package.json`, Phase 5); `make test-uc3` covers only atlassian-remote + atlassian-agent and is green.
...
*Last updated: 2026-06-21 by Ahmed Ben Rejeb (via Claude Code) — added the **UC3 Semantic Duplicate Resolver**: new shared `DuplicateVerdict` model + `DUPLICATE_CONFIDENCE_THRESHOLD` (0.85) constant in `trace-core` (py + ts mirror); `atlassian-remote` `duplicate_resolver.py` (CF Workers AI judge reusing `rca_generator._extract_json`/`_format_hits`), `analyzer.resolve_incident_duplicate`, `DuplicateResult` envelope, and a `POST /duplicates` endpoint (same `X-Sentinel-Secret` auth); `atlassian-agent` 7th action `resolve-duplicate` that auto-links Jira issues (`linkIssues` → `POST /rest/api/3/issueLink`, type via `ATLASSIAN_DUPLICATE_LINK_TYPE`) + posts a polite ADF comment when confident (≥0.85), else surfaces candidates for a human. `make test-uc3` green (38 Python + 25 TS); ruff/black/isort/mypy --strict + tsc/eslint/prettier + check-docs clean. Branch `feat/uc3-semantic-duplicate-resolver`. Earlier: 2026-06-19 — built the **UC3 Phase 3 cores**: `atlassian-remote` (FastAPI :8080 — `/analyze` `/search` `/embed` `/health`; CF Workers AI embed+chat, xqdrant `query_points` search with attribution + 0.75 threshold, RCA → `RcaDraft`, Atlassian REST client with 429 backoff; 30 pytest green, ruff/black/isort/mypy --strict + check-docs clean) and `atlassian-agent` (Forge Rovo agent + 6 actions, ADF everywhere, Forge-Remote calls with `X-Sentinel-Secret`/`X-Account-Id`; 19 jest tests green, tsc/eslint/prettier clean). Added `packages/atlassian-remote` to the uv workspace + `make typecheck`, a `qdrant_client.*` mypy override, and a root `pnpm-workspace.yaml`. UC3 not yet deployed to Forge (needs `forge register`/`deploy`). Earlier: `AGENTS.md` symlink + `make check-docs`; Phase 1 + Phase 2 cores green.*